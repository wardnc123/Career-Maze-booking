...
export async function createBooking(data: BookingRequest & { vpAlias?: string }): Promise<BookingResult> {
  ...
  try {
    ...
    if (session.bookingCount < session.maxAttendees) {
      const booking: Booking = { id: uuidv4(), sessionId: data.sessionId, name: data.name, email: data.email, role: data.role, pf: data.pf, status: 'confirmed', referenceCode: generateReferenceCode(), customFields: (data as any).customFields || null, vpAlias: data.vpAlias || '', attended: false, createdAt: new Date(), cancelledAt: null };
